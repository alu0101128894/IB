/*
Ejercicio 1: Se quiere almacenar la siguiente informaci�n sobre libros de una librer�a:
t�tulo, editorial, precio, ISBN. Se pide implementar un programa que, haciendo uso de
las sentencias de control condicionales, solicite la informaci�n al usuario para 3 libros.
Para los libros cuyo precio sea inferior a 20 euros se les aplicar� un descuento de un 15% en el precio,
para los que su precio supere los 80 euros el descuento ser� de un 30% y el resto de libros mantendr� su precio.
A continuaci�n el programa debe mostrar, para todos los libros, sus datos actualizados.
*/

/* Modificacion: A los libros cuyo precio es >20 y >80 descuento del 10% */
...
int main(void){
	...
	if(libros[0].p < 20){																                             //Precio 1
		...
		libros[0].pd = libros[0].p - (libros[0].p * 0.10);
	}
	...
	if(libros[1].p < 20){																                             //Precio 2
		...
		libros[1].pd = libros[1].p - (libros[1].p * 0.10);
	}
	...
	if(libros[2].p < 20){																                              //Precio 3
		...
		libros[2].pd = libros[2].p - (libros[2].p * 0.10);
	}
