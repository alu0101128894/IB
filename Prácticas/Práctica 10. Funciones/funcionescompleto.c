...
int diagonalprincipal (int a, int b, int matriz[a][b]){
	
	int i,j,mediaprincipal;

	for(i=0; i<a; i++){
		for(j=0; j<a; j++){
			if(i == j){
				mediaprincipal += matriz[i][j];
			}
		}
	}
	mediaprincipal = mediaprincipal / a;
...
int main (void){
	...
	do{
    	...
    	printf("4: Cacular la media de la diagonal principal de una de las matrices.\n");
    	printf("5: Calcular el producto de las matrices.\n");
    	printf("6: Salir.\n");
		...
		if(opcion == 4){		// ---------> MEDIA DIAGONAL PRINCIPAL
			printf("\nElija matriz 1 o 2: ");
    		scanf("%d", &opcion3); 
    	
    		if(x == y){
    			if (opcion3 == 1){
					printf("\nLa media de la diagonal principal de la primera matriz es: %d\n",diagonalprincipal (x, y, matriz));   //Primera matriz
				}
			}
			else if(x != y){
				printf("\n\tnNo es una matriz cuadrada\n");

			}
			if(z == t){
    			if (opcion3 == 2){
					printf("\nLa media de la diagonal principal de la segunda matriz es: %d\n",diagonalprincipal (z, t, matriz2));	//Segunda matriz
				}
			}
			else if(z != t){
				printf("\nNo es una matriz cuadrada\n");
